...
class ValidationTest(unittest.TestCase):
    def test_from_categorical(self):
        print('Testing evaluation.from_categorical')
        x = np.array([
            [0, 0, 1],
            [0, 1, 0],
            [1, 0, 0]
        ...
        y = [2, 1, 0]
        self.assertEqual(evaluation.from_categorical(x), y)

    def test_extract_validation(self):
        print('Testing evaluation.extract_validation')
        x1 = [0, 2, 0, 1, 1, 3, 1, 2, 0, 3]
        x2 = [0, 2, 0, 1, 2, 3, 1, 2, 0, 2]

        (acc, accs, matrix) = evaluation.extract_validation(x1, x2, categorical=False)
